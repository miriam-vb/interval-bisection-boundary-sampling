...
thresh_forest.num <- function(thresh_obj,
  ...
  # import data and arguments from bias_thresh_1D function output
  thresh.df <- thresh_obj$thresh.df
  admin <- thresh_obj$args$admin
  
  # rename threshold data frame
  colnames(thresh.df) <- c("lo", "lo.newkstar", "hi","hi.newkstar")
  ...
  # nmathresh.num specific function to control printing of II boundaries with   
  # consideration of Admin cutoffs
  printsigII <- function(num, kstar, dig = ifelse(sf, sigfig, digits), cutyn = FALSE) {
    if (cutyn && (num >= min(Inf, cutoff[2]) || 
                  num <= max(-Inf, cutoff[1]) || kstar == "Admin")) 
      "NT"
    else if (sf) gsub("\\.$", "", formatC(signif(num, digits = dig), digits = dig, format = "fg", flag = "#"))
    else formatC(num, dig, format = "f")
  }
  ...
  g_for <- mapply(forestgrob,
                  ...
                  MoreArgs = list(xlim, CI.lwd, II.cols, II.colw, II.lwd, pointsize, admin),
                  SIMPLIFY = FALSE)
...
forestgrob <- function(y,
                       ...
                       xlim, CI.lwd, II.cols, II.colw, II.lwd, pointsize, admin){
  ...
  # Boolean introduced to determine whether the invariant interval endpoint
  # is an identified bias threshold or an administrative cutoff value
  isAd.lo <- abs(II.lo - y) == admin
  
  tII.hi <- min(II.hi, xlim[2])
  istII.hi <- II.hi > xlim[2]
  # Boolean introduced to determine whether the invariant interval endpoint
  # is an identified bias threshold or an administrative cutoff value
  isAd.hi <- abs(II.hi - y) == admin
